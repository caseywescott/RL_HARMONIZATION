...
app = FastAPI(title="Final Corrected Coconet Server")
...
@app.get("/status")
async def status():
    return {"status": "running", "model": "final-corrected-coconet"}

def run_final_corrected_harmonization(input_midi_path: str, output_dir: str, temperature: float):
    """Run Coconet harmonization with proper melody preservation"""
    try:
        print(f"   Running final corrected Coconet harmonization...")

        # Create a temporary Python script with the proper approach
        final_script_content = f'''
#!/usr/bin/env python3
...
# Create a proper melody-preserving masker that works with Coconet's design
class FinalMelodyPreservingMasker(lib_sampling.BaseMasker):
    """Properly preserves melody while allowing harmony generation.
    
    Coconet treats all parts as potentially modifiable unless explicitly constrained.
    We need to ensure the melody (instrument 0) is preserved while allowing
    harmony (instruments 1-3) to be generated.
    """
    key = "final_melody_preserving"
...
# Override the HarmonizeMidiMelodyStrategy to use our final masker
class FinalHarmonizeMidiMelodyStrategy(HarmonizeMidiMelodyStrategy):
    def run(self, tuple_in):
        ...
        # Store the original melody for verification
        original_melody = pianorolls[0, :, :, 0].copy()

        # Use our final masker to ensure melody preservation
        masks = FinalMelodyPreservingMasker()(pianorolls.shape)
        ...
        # Ensure melody is preserved by explicitly restoring it if needed
        final_melody = pianorolls[0, :, :, 0]
        if not np.array_equal(original_melody, final_melody):
            print("   ⚠️  Melody was modified, restoring original melody...")
...
if __name__ == "__main__":
    ...
    HarmonizeMidiMelodyStrategy = FinalHarmonizeMidiMelodyStrategy

    # Run the original main function
    tf.app.run()
'''

        # Write the final script to a temporary file
        with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
            f.write(final_script_content)
            final_script_path = f.name

        try:
            # Run the final Coconet script
            command = [
        ...
        print(f"   ❌ Error in final harmonization: {e}")
        raise
...
async def harmonize_melody(
    ...
    """Harmonize a melody using Coconet with proper melody preservation."""
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            harmonized_file = run_final_corrected_harmonization(input_path, output_dir, temperature)

            # Return the harmonized file
            return FileResponse(
                harmonized_file,
                media_type="audio/midi",
                filename=f"final_harmonization_temp_{temperature}.mid"
            )
